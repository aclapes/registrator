// dualCalibration.cpp : Defines the entry point for the console application.
//
...
float Registrator::lookUpDepth(Mat depthImg, Point2f dCoord, bool SCALE_TO_THEORETICAL)
{
	...
	depthInMm = (float)depthImg.at<unsigned short>(dCoord.y-1,dCoord.x-1)*0.1;
...
void Registrator::computeCorrespondingRgbPointFromDepth(vector<Point2f> vecDCoord,vector<Point2f> & vecRgbCoord)
{
	...
	for (int i = 0; i < vecDCoord.size(); i++)
	{
		if ((vecDCoord[i].y > 0) && (vecDCoord[i].x > 0) && (vecDCoord[i].y <= stereoCalibParam.HEIGHT) && (vecDCoord[i].x <= stereoCalibParam.WIDTH)) {
			tmpPoint.x = stereoCalibParam.dToRgbCalX.at<short>(vecDCoord[i].y-1,vecDCoord[i].x-1); // Look up the corresponding x-point in the depth image
			tmpPoint.y = stereoCalibParam.dToRgbCalY.at<short>(vecDCoord[i].y-1,vecDCoord[i].x-1); // Look up the corresponding y-point in the depth image
...
void Registrator::computeCorrespondingDepthPointFromRgb(vector<Point2f> vecRgbCoord,vector<Point2f> & vecDCoord)
{
	...
	for (int i = 0; i < vecDistRgbCoord.size(); i++) 
	{
		if ((vecDistRgbCoord[i].y > 0) && (vecDistRgbCoord[i].x > 0) && (vecDistRgbCoord[i].y <= stereoCalibParam.HEIGHT) 
			...
			tmpPoint.x = stereoCalibParam.rgbToDCalX.at<short>(vecDistRgbCoord[i].y-1,vecDistRgbCoord[i].x-1); // Look up the corresponding x-point in the depth image
			tmpPoint.y = stereoCalibParam.rgbToDCalY.at<short>(vecDistRgbCoord[i].y-1,vecDistRgbCoord[i].x-1); // Look up the corresponding y-point in the depth image
			...
			if ((tmpPoint.x == 0) || (tmpPoint.y == 480) || (tmpPoint.y == 0)) {
				...
				for (int j = 0; j < 4; ++j) {
					...
					if (((vecDistRgbCoord[i].y+yOffset) > 0) && ((vecDistRgbCoord[i].x+xOffset) > 0) && ((vecDistRgbCoord[i].y+yOffset) <= stereoCalibParam.HEIGHT) 
						...
						tmpLookupPoint.x = stereoCalibParam.rgbToDCalX.at<short>(vecDistRgbCoord[i].y-1+yOffset,vecDistRgbCoord[i].x-1+xOffset); // Look up the corresponding x-point in the depth image
						tmpLookupPoint.y = stereoCalibParam.rgbToDCalY.at<short>(vecDistRgbCoord[i].y-1+yOffset,vecDistRgbCoord[i].x-1+xOffset); // Look up the corresponding y-point in the depth image
...
void Registrator::computeCorrespondingThermalPointFromRgb(vector<Point2f> vecRgbCoord, vector<Point2f>& vecTCoord, vector<Point2f> vecDCoord)
{
	...
	vector<double> depthRectDisplacement;
	vector<vector<double>> octantDistances;
	vector<vector<int>> octantIndices;
	vector<Point3f> worldCoordPointVector;
	computeCorrespondingThermalPointFromRgb(vecRgbCoord, vecTCoord, vecDCoord, vecDepthInMm, depthRectDisplacement, minDist, 
											bestHom, octantIndices, octantDistances, worldCoordPointVector);

}

void Registrator::computeCorrespondingThermalPointFromRgb(vector<Point2f> vecRgbCoord, vector<Point2f>& vecTCoord, vector<Point2f> vecDCoord, 
		vector<double>& depthRectDisplacement, vector<int> &bestHom)
{
...
void Registrator::computeCorrespondingThermalPointFromRgb(vector<Point2f> vecRgbCoord, vector<Point2f>& vecTCoord, vector<Point2f> vecDCoord, 
											vector<int> vecDepthInMm, vector<double>& depthRectDisplacement, vector<double>& minDist, 
											...
											vector<Point3f> &worldCoordPointVector)
{
	...
	Point2f tmpPoint;
	depthRectDisplacement.clear(); minDist.clear(); bestHom.clear();
	...
	*/
	...
	for (auto i = 0; i < homWeights.size(); ++i)
	{
		if (homWeights[i] > 0)
		{
			...
			estimatedPoint[0].x = estimatedPoint[0].x + tmpEstimatedPoint[0].x * homWeights[i];
			estimatedPoint[0].y = estimatedPoint[0].y + tmpEstimatedPoint[0].y * homWeights[i];
		}
	}
...
void Registrator::depthOutlierRemovalLookup(vector<Point2f> vecDCoord, vector<int> &vecDepthInMm)
{
	...
	int avgDepthInMm, movingAvgDepthInMm;
	vecDepthInMm.clear();
	vector<int> rawDepthInMm;

	if (SCALE_TO_THEORETICAL) {
		depthNaNThreshold = stereoCalibParam.depthCoeffA*6500+stereoCalibParam.depthCoeffB;
	} else {
	...
	if (vecDCoord.size() > 0)
	{
		...
		for (size_t i = 0; i < vecDCoord.size(); ++i)
		{
			if (vecDCoord.size() > 0)  // Are we providing any depth coordinates (as in the case of thermal coordinates)
			{
				if ((vecDCoord[i].y > 0) && (vecDCoord[i].x > 0) && (vecDCoord[i].y <= stereoCalibParam.HEIGHT) 
						&& (vecDCoord[i].x <= stereoCalibParam.WIDTH))
				{
					depthInMm =  lookUpDepth(dImg, vecDCoord[i], SCALE_TO_THEORETICAL);
		...
		if (vecDCoord.size() > nbrClusters) {
			...
			for (int i = 0; i < nbrClusters; ++i) {
				centersVec.push_back(centers.at<float>(i,0));
			}
		} else {
...
void Registrator::loadRegSaveContours()
{
	...
	string rgbContourPath = "P:/Private/Dataset/Annotation/Annotation - Without dontcare - Registered/Scene 1/rgbMasks/";
	string depthContourPath = "P:/Private/Dataset/Annotation/Annotation - Without dontcare - Registered/Scene 1/depthMasks/";
	string thermalContourPath = "P:/Private/Dataset/Annotation/Annotation - Without dontcare - Registered/Scene 1/thermalMasks/";
	string depthImagePath = "P:/Private/Dataset/Annotation/Annotation - Without dontcare - Registered/Scene 1/SyncD/";
...
void Registrator::getRegisteredContours(vector<Point> contour, vector<Point> erodedContour, vector<Point>& dContour, vector<Point>& tContour, Mat depthImg)
{
	...
	if (rgbCoords.size() > 0) {
		this->computeCorrespondingThermalPointFromRgb(rgbCoords, tCoords, dCoords, vecDepthInMm, depthRectDisplacement, minDist, 
...
{
	vector<Point2f> vecTCoord,vecDCoord,epiTCoord,vecDistRgbCoord,vecRgbCoord,tempVecRgbCoord,undistAgain;
	Point2f tCoord,dCoord,distRgbCoord;
	Mat tmpDImg;
	vecRgbCoord.push_back(rgbCoord);
	tempVecRgbCoord = vecRgbCoord;
	vector<double> depthRectDisplacement;
	vector<int> homInd;
	...
	computeCorrespondingThermalPointFromRgb(vecRgbCoord, vecTCoord, vecDCoord,depthRectDisplacement, homInd);

	tCoord = vecTCoord[0];
	if (depthRectDisplacement.size() == 0)
	{
		depthRectDisplacement.push_back(0);
	}
	...
	markCorrespondingPoints(rgbCoord, tCoord, dCoord, depthRectDisplacement, homInd[0], MAP_FROM_RGB );
...
void Registrator::markCorrespondingPointsThermal(Point2f tCoord)
{
	...
	markCorrespondingPoints(rgbCoord, tCoord, dCoord, depthRectDisplacement, bestHom[0], MAP_FROM_THERMAL);
...
void Registrator::markCorrespondingPointsDepth(Point2f dCoord)
{
	...
	computeCorrespondingThermalPointFromRgb(vecRgbCoord, vecTCoord, vecDCoord,depthRectDisplacement, bestHom);
	tCoord = vecTCoord[0];
	...
	markCorrespondingPoints(rgbCoord, tCoord, dCoord, depthRectDisplacement, bestHom[0], MAP_FROM_DEPTH);
}

void Registrator::markCorrespondingPoints(Point2f rgbCoord, Point2f tCoord, Point2f dCoord, vector<double> depthRectDisplacement, int homInd, int MAP_TYPE)
{
	...
	Mat tmpDImg;
	dImg.copyTo(tmpDImg);
	circle(tmpDImg, dCoord, 2, dColor, -1);
	imshow("Depth",tmpDImg);
...
void Registrator::toogleUndistortion(bool undistort)
{
...
{	
	...
	minReg.loadMinCalibrationVars("P:/Private/Workset/Capture/1/Cal/1/CalibrationHelper/calibVars.yml");
	minReg.setRgbImgPath("P:/Private/Workset/Capture/1/SyncRGB/");
	minReg.setTImgPath("P:/Private/Workset/Capture/1/SyncT/");
	minReg.setDImgPath("P:/Private/Workset/Capture/1/SyncD/");
	minReg.toogleUndistortion(false);
